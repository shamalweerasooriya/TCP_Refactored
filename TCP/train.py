...
class TCP_planner(pl.LightningModule):
	...
	def training_step(self, batch, batch_idx):
		...
		if(batch_idx==0):
			...
			self.ref_front_img_all_o = front_img_o
		...
		action_loss = torch.mean(kl_div[:, 0]) * (1 - self.config.depth_branch_weight)/2 + torch.mean(kl_div[:, 1]) * (1 - self.config.depth_branch_weight)/2 +
		              torch.mean(kl_div_depth[:, 0]) * self.config.depth_branch_weight + torch.mean(kl_div_depth[:, 1]) * self.config.depth_branch_weight
		...
		feature_loss = ((F.mse_loss(pred['pred_features_traj'], feature) +F.mse_loss(pred['pred_features_ctrl'], feature))* (1 - self.config.depth_branch_weight) +
					   (F.mse_loss(pred['pred_depth_features_traj'], feature) +F.mse_loss(pred['pred_depth_features_ctrl'], feature)) * self.config.depth_branch_weight) * self.config.features_weight
		wp_loss = F.l1_loss(pred['pred_wp'], gt_waypoints, reduction='none').mean() * (1 - self.config.depth_branch_weight) + 
				  F.l1_loss(pred['pred_depth_wp'], gt_waypoints, reduction='none').mean() * self.config.depth_branch_weight
		...
		for i in range(self.config.pred_len):
			...
			kl_div = torch.distributions.kl_divergence(dist_sup, dist_pred)
			kl_div_depth = torch.distributions.kl_divergence(dist_sup, dist_depth_pred)
			future_action_loss += torch.mean(kl_div[:, 0]) * (1 - self.config.depth_branch_weight)/2 + torch.mean(kl_div[:, 1]) * (1 - self.config.depth_branch_weight)
								  torch.mean(kl_div_depth[:, 0]) * self.config.depth_branch_weight + torch.mean(kl_div_depth[:, 1]) * self.config.depth_branch_weight
		...
		loss = action_loss + action_loss_depth + speed_loss + value_loss + feature_loss + wp_loss + future_feature_loss + future_action_loss
	...
	def validation_step(self, batch, batch_idx):
		...
		kl_div = torch.distributions.kl_divergence(dist_sup, dist_pred)
		kl_div_depth = torch.distributions.kl_divergence(dist_sup, dist_depth_pred)
		action_loss = torch.mean(kl_div[:, 0]) * (1 - self.config.depth_branch_weight)/2 + 
					  torch.mean(kl_div[:, 1]) * (1 - self.config.depth_branch_weight)/2  + 
					  torch.mean(kl_div_depth[:, 0]) * (self.config.depth_branch_weight)/2  + 
					  torch.mean(kl_div_depth[:, 1]) * (self.config.depth_branch_weight)/2 
		speed_loss = F.l1_loss(pred['pred_speed'], speed) * self.config.speed_weight
		value_loss = ((F.mse_loss(pred['pred_value_traj'], value) + F.mse_loss(pred['pred_value_ctrl'], value)) * (1 - self.config.depth_branch_weight) +
					 (F.mse_loss(pred['pred_depth_value_traj'], value) + F.mse_loss(pred['pred_depth_value_ctrl'], value)) * self.config.depth_branch_weight) * self.config.value_weight
		feature_loss = ((F.mse_loss(pred['pred_features_traj'], feature) +F.mse_loss(pred['pred_features_ctrl'], feature))* (1 - self.config.depth_branch_weight) +
		...
		batch_steer_l1_d = 0 
		batch_brake_l1_d = 0
		batch_throttle_l1_d = 0
		for i in range(B):
			throttle, steer, brake = self.model.get_action(pred['mu_branches'][i], pred['sigma_branches'][i])
			throttle_depth, steer_depth, brake_depth = self.model.get_action(pred['mu_depth_branches'][i], pred['sigma_depth_branches'][i])
			...
			batch_throttle_l1_d += torch.abs(throttle_depth-batch['action'][i][0])
			batch_steer_l1_d += torch.abs(steer_depth-batch['action'][i][1])
			batch_brake_l1_d += torch.abs(brake_depth-batch['action'][i][2])

		batch_throttle_l1 /= B
		batch_steer_l1 /= B
		batch_brake_l1 /= B
		batch_throttle_l1_d /= B
		batch_steer_l1_d /= B
		batch_brake_l1_d /= B
		...
		for i in range(self.config.pred_len-1):
			...
			kl_div = torch.distributions.kl_divergence(dist_sup, dist_pred)
			kl_div_depth = torch.distributions.kl_divergence(dist_sup, dist_depth_pred)
			future_action_loss += torch.mean(kl_div[:, 0]) *(1 - self.config.depth_branch_weight)/2 + torch.mean(kl_div[:, 1]) *(1 - self.config.depth_branch_weight)/2 + 
								  torch.mean(kl_div_depth[:, 0]) *self.config.depth_branch_weight + torch.mean(kl_div_depth[:, 1]) *self.config.depth_branch_weight
			future_feature_loss += (F.mse_loss(pred['future_feature'][i], batch['future_feature'][i]) * (1 - self.config.depth_branch_weight) + 
									F.mse_loss(pred['future_depth_feature'][i], batch['future_feature'][i]) * self.config.depth_branch_weight) * self.config.features_weight
		future_feature_loss /= self.config.pred_len
		...
		val_loss = wp_loss + batch_throttle_l1+5*batch_steer_l1+batch_brake_l1 + batch_throttle_l1_d+5*batch_steer_l1_d+batch_brake_l1_d
		...
		self.log('val_future_feature_loss', future_depth_feature_loss.item(), sync_dist=True)
		self.log('val_future_action_loss', future_depth_action_loss.item(), sync_dist=True)
	...
	def visActivations(self, img, img_o, state, target_point):
		...
		feature_emb = torch.sum(cnn_feature*init_att, dim=(2, 3))
		j_ctrl = self.model.join_ctrl(torch.cat([feature_emb, measurement_feature], 1))
		test_output['pred_depth_value_ctrl'] = self.model.value_branch_ctrl(j_ctrl)
		test_output['pred_depth_features_ctrl'] = j_ctrl
		policy = self.model.policy_head(j_ctrl)
		test_output['mu_depth_branches'] = self.model.dist_mu(policy)
		test_output['sigma_depth_branches'] = self.model.dist_sigma(policy)

		x = j_ctrl
		mu = test_output['mu_depth_branches']
		sigma = test_output['sigma_depth_branches']
		future_feature, future_mu, future_sigma = [], [], []
		...
		for _ in range(self.model.config.pred_len):
			x_in = torch.cat([x, mu, sigma], dim=1)
			h = self.model.decoder_ctrl(x_in, h)
			wp_att = self.model.wp_att(torch.cat([h, depth_traj_hidden_state[:, _]], 1)).view(-1, 1, 8, 29)
			attention_map = torch.Tensor.cpu(wp_att.squeeze()).detach()
			self.logger[0].experiment.add_image("attention_map_depth" + str(l), attention_map, self.current_epoch, dataformats="HW")
			new_feature_emb = torch.sum(cnn_feature*wp_att, dim=(2, 3))
			merged_feature = self.model.merge(torch.cat([h, new_feature_emb], 1))
			dx = self.model.output_ctrl(merged_feature)
			x = dx + x
		...
		test_output['future_depth_feature'] = future_feature
		test_output['future_depth_mu'] = future_mu
		test_output['future_depth_sigma'] = future_sigma
